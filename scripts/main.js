...
function openPopupCallback(imageUrl, imageAlt) {
  const zoomPopup = document.querySelector('#popup_image');
  const zoomImg = zoomPopup.querySelector('.popup__image');
  const zoomImgTitle = zoomPopup.querySelector('.popup__image-title');

  // Заполняем контент попапа перед открытием
  zoomImg.src = imageUrl;
  zoomImg.alt = imageAlt;
  zoomImgTitle.textContent = imageAlt;

  openPopup(zoomPopup); // Вызываем функцию открытия попапа
}
